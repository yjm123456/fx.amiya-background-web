  ...
  <div class="content">
    ...
    >
      <div class="con" v-for="(item,index) in messageBoardPage.data" :key="index">
        <div class="item" >
          <!-- item.belongAuthorize == 0是管理员端  1是医院端 -->
          <div class="item_left">
            <img
              :src="item.avatar ? item.avatar : logoMin"
              alt=""
              class="img"
              v-if="item.belongAuthorize == 0"
            />
            <img
              :src="item.avatar ? item.avatar :logoMin"
              alt=""
              ...
              v-else
            />
            <div class="name" v-if="item.belongAuthorize == 0">{{item.employeeName ? item.employeeName :'啊美雅'}}</div>
            <div class="name" v-else>{{item.employeeName ? item.employeeName : hospitalName}}</div>
          </div>
          <div class="time">{{$moment(item.createDate).format('YYYY-MM-DD HH:mm:ss')}}</div>
        </div>
...
export default {
  ...
  methods: {
    ...
    getMessageBoardList() {
      ...
      api.allCustomerListWithPage(data).then((res) => {
        if (res.code === 0) {
          const { list, totalCount } = res.data.orderRemark;
