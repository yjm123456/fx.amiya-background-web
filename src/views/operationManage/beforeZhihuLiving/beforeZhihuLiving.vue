  ...
  <div>
    ...
    >
      ...
      >
        ...
        <Row :gutter="30">
          <Col span="8">
            <FormItem label="知乎运营人员" prop="zhihuOperationEmployeeId">
              <Select
                v-model="form.zhihuOperationEmployeeId"
                placeholder="请选择知乎运营人员"
                filterable
              >
                <Option
          ...
          <Col span="8">
            <FormItem label="知乎今日投流费用" prop="zhihuFlowInvestmentNum">
              <Input
                v-model="form.zhihuFlowInvestmentNum"
                placeholder="请输入知乎今日投流费用"
                type="number"
                number
                @on-change="zhihuFlowInvestmentNumChange"
              />
            </FormItem>
          </Col>
        </Row>
...
export default {
                      ...
                      click: () => {
                        ...
                        api.byIdLiveAnchorDailyTarget(id).then((res) => {
                          if (res.code === 0) {
                            ...
                            this.form.zhihuOperationEmployeeId = zhihuOperationEmployeeId==0 ?  null : zhihuOperationEmployeeId ;
                            this.form.zhihuSendNum = zhihuSendNum;
                            this.form.zhihuFlowInvestmentNum = zhihuFlowInvestmentNum;
                            this.form.alltodaySendNum = Number(tikTokSendNum)+Number(xiaoHongShuSendNum)+Number(sinaWeiBoSendNum)+Number(videoSendNum)
                            ...
                            +Number(videoFlowInvestmentNum) ) * 100) / 100;
  ...
  methods: {
    zhihuSendNumChange(){
      this.form.todaySendNum = Number(this.form.zhihuSendNum) 
    },
    zhihuFlowInvestmentNumChange(){
      this.form.flowInvestmentNum = Number(this.form.zhihuFlowInvestmentNum)
    },
    yearChange() {
      this.getLiveAnchorMonthlyTarget();
    },
        ...
        if (valid) {
          if (this.isEdit) {
            ...
            const data = {
              id,
              liveanchorMonthlyTargetId,
              zhihuOperationEmployeeId: zhihuOperationEmployeeId
                ? zhihuOperationEmployeeId
                : 0,
              zhihuSendNum: zhihuSendNum ? zhihuSendNum : 0,
              zhihuFlowInvestmentNum: zhihuFlowInvestmentNum ? zhihuFlowInvestmentNum : 0,
              todaySendNum:Number(alltodaySendNum) + Number(zhihuSendNum),
              flowInvestmentNum:Math.floor((allflowInvestmentNum + zhihuFlowInvestmentNum) * 100) /100,
              recordDate: this.$moment(new Date(recordDate)).format(
                "YYYY-MM-DD"
              ),
            };
            api.BeforeLivingZhihuUpdate(data).then((res) => {
              if (res.code === 0) {
                this.isEdit = false;
            ...
            // 添加
            api.BeforeZhihuLivingAdd(data).then((res) => {
              if (res.code === 0) {
                this.cancelSubmit("form");
                this.getLiveAnchorDayList();
                this.$Message.success({
