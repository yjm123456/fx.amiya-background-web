  ...
  <div>
    ...
    >
      <Form
        ...
        :label-width="110"
      >
        <FormItem label="物流公司" prop="expressName">
          <Input v-model="form.expressName"></Input>
        </FormItem>
        <FormItem label="快递100Code" prop="expressCode">
          <Input v-model="form.expressCode"></Input>
        </FormItem>
...
export default {
                  ...
                  "Button",
                  {
                    ...
                    on: {
                      click: () => {
                        const { id } = params.row;
                        this.title = "修改";
                        api.byIdExpressMange(id).then((res) => {
                          if (res.code === 0) {
                             const {
                              // 快递公司名称
                              expressName,
                            ...
                            this.form.expressName = expressName;
                            this.form.expressCode = expressCode;
                            this.form.id = id;
  ...
  methods: {
    ...
    getHospitalInfo() {
      ...
      api.getLogisticsCompanyList(data).then((res) => {
        if (res.code === 0) {
          const { list, totalCount } = res.data.expressInfo;
