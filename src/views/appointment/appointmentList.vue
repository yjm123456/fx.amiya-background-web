<template>
  <div>
    ...
    <Card class="container">
      <div>
        <Table border :columns="employeeType == 'amiyaEmployee' ? query.columns :query.hospitalColumns" :data="query.data" ></Table>
      </div>
      <div class="page_wrap">
        <Page
          ref="pages"
    ...
    >
      ...
      <div slot="footer">
        ...
        <Button type="primary" @click="distributeLeafletsHandleSubmit('distributeLeafletsForm')">确定</Button>
      </div>
    </Modal>
    <Modal
      v-model="remarkModel"
  ...
  methods: {
    ...
    handleSubmit(name) {
       this.$refs[name].validate((valid) => {
        if (valid) {
          if (this.isEdit) {
              const {id,appointmentDate,week,time,itemName,phone} = this.form
              const data = {
                id,
                appointmentDate:this.$moment(appointmentDate).format("YYYY-MM-DD"),
                week,time,itemName,phone
              }
              if(phone){
