    ...
    >
      ...
      >
        ...
        <FormItem label="派单数" prop="sendOrderNum">
          <Input v-model="form.sendOrderNum" placeholder="请输入派单数" type="number" number></Input>
        </FormItem>
...
export default {
                  ...
                  "Button",
                  {
                    ...
                    on: {
                      click: () => {
                        const { id } = params.row;
                        this.title = "修改";
                        api.byIdHospitalNetWorkConsulationOperationDatas(id).then((res) => {
                          if (res.code === 0) {
                             const {
                              id,
                              indicatorId,
                    ...
                    on: {
                      click: () => {
                        this.$Modal.confirm({
                          ...
                          onOk: () => {
                            const { id } = params.row;
                            api.deleteHospitalNetWorkConsulationOperationDatas(id).then((res) => {
                              if (res.code === 0) {
  ...
  methods: {
    ...
    submitClick(){
      const {amiyaOnlineConsultRemark,hospitalOnlineConsultRemark} = this.query
      const data = {
        indicatorId:this.indicatorsId,
        hospitalId:this.hospitalId,
        hospitalOnlineConsultRemark,
      ...
      this.$refs[name].validate((valid) => {
        if (valid) {
          if (this.isEdit) {
            ...
            const { consulationName, sendOrderNum, newCustomerVisitNum,newCustomerVisitRate,id } = this.form;
            const  data = { 
                consulationName ,
                sendOrderNum,
                newCustomerVisitNum,
            ...
            // 修改
            api.editHospitalNetWorkConsulationOperationDatas(data).then((res) => {
              if (res.code === 0) {
                this.isEdit = false;
            ...
            let  indicatorsId = this.indicatorsId
            const { consulationName, sendOrderNum, newCustomerVisitNum,newCustomerVisitRate } = this.form;
            const  data = { 
                consulationName ,
                sendOrderNum,
                newCustomerVisitNum,
            ...
            // 添加
            api.addHospitalNetWorkConsulationOperationDatas(data).then((res) => {
              if (res.code === 0) {
                this.cancelSubmit("form");
                this.getHospitalInfo();
                this.$Message.success({
