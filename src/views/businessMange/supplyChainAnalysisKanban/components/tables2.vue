<template>
  <div class="con">
        <Table border :columns="query.columns" :data="query.data" :row-class-name="rowClassName2" height="350" class="table"></Table>
  </div>
</template>
<script>
export default {
  ...
  watch: {
    performanceKanbanParams: {
      handler(value) {
        const {topCreateOrderGMVDtoList}  = value.itemDetailsAnalizeData
