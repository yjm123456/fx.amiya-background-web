        ...
        <Row :gutter="30">
          ...
          <Col span="8">
            <FormItem label="主播平台" prop="contentPlateFormId">
              <Select
                ...
                filterable
                :disabled="
                  recordingNormalParams.title == '录单编辑' && buttonFlag == false
                "
              >
                <Option
          ...
          <Col span="8">
            <FormItem label="主播微信号" prop="liveAnchorWeChatNo">
              ...
              >
                <Option
                  v-for="(item,indexs) in recordingParams.weChatList"
                  :value="item.id"
                  :key="indexs"
  ...
  methods: {
    ...
    handleSubmit(name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          if (this.recordingNormalParams.title == '录单编辑') {
            // 修改
            const {
              orderType,
              contentPlateFormId,
            ...
            if (phone) {
              ...
              api.editContentPlateFormOrder(data).then((res) => {
                ...
                this.$parent.$parent.handlePageChange(this.$parent.$parent.$refs.pages.currentPage)
    ...
    handleCancel(name) {
      ...
      this.$emit("update:recordingNormalModel", false);
    },
    // modal 显示状态发生变化时触发
