      ...
      <Card class="container">
        ...
        <div slot="footer" class=" foot">
          <div  style="display:flex">
            ...
            <div class="num">合计定金金额：<div style="color:red">{{depositAmount}}</div></div>
            <div class="num">合计成交金额：<div style="color:red">{{dealAmount}}</div></div>
...
export default {
      ...
      // 主播IP账号
        ...
        query:{
          ...
          },
          {
            ...
            minWidth: 180,
            render: (h, params) => {
              return params.row.dealDate ? h("div",this.$moment(params.row.dealDate).format("YYYY-MM-DD HH:mm:ss")) : '';
            },
          },
          {
            title: "电话",
  ...
  methods:{
    ...
    ContentPlatFormOrderDealReport() {
      const { startDate,endDate ,checkState,liveAnchorId,dealHospitalId} = this.query;
      const data = { 
    ...
    exportsendOrder(){
      ...
      api.ExportContentPlatFormOrderDealReport(data).then((res) => {
        let name = this.$moment(new Date(startDate)).format("YYYY-MM-DD") + '-' + this.$moment(new Date(endDate)).format("YYYY-MM-DD") + '内容平台成交报表'
        download(res,name);
      });
    },
