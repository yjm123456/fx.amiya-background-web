<template>
    <Modal
        v-model="newCustomerDealRateModels"
        title="新客成交率"
        :mask-closable="false"
...
export default {
    props:{
        newCustomerDealRateModel:Boolean,
        newCustomerDealRateList:Array,
        hospital:String,
        date:String,
    },
    components:{
    ...
    data(){
        return{
            newCustomerDealRateModels:false,
            myChart: "",
        }
    },
    methods:{
        cancel(){
            this.$emit("update:newCustomerDealRateModel", false);
        },
        // modal 显示状态发生变化时触发
    ...
    watch: {
        newCustomerDealRateModel: {
            handler(value) {
                this.newCustomerDealRateModels = value
            },
            deep: true,
        },
        newCustomerDealRateList(value){
