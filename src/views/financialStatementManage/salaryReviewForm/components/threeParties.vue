<template>
  <div class="content">
    <Modal
      ...
      width="60%"
    >
      <div class="model_title">  -（三方订单数据）</div>
      <Form
        ref="form"
        :model="form"
        :rules="ruleValidates"
...
export default {
  ...
  methods: {
    handleSubmit(name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          ...
          this.isLoading = true;
          api.batchCheckConsumptionRracking(data).then((res) => {
            if (res.code === 0) {
    ...
    handleCancel(name) {
      this.$emit("update:threePartiesModel", false);
