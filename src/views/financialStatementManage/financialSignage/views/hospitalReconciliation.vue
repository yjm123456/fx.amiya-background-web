<template>
  <div>
    <Card :dis-hover="true">
      ...
      ></DatePicker>
      <Button type="primary" style="margin-left: 10px" @click="gethospitalDealPriceBoard()"
        >查询</Button
      >
    </Card>
...
export default {
  ...
  methods: {
    handlePageSizeChange(pageSize){
      ...
      this.gethospitalDealPriceBoard()
    },
    // 获取医院对账看板列表
    gethospitalDealPriceBoard() {
      this.$nextTick(() => {
      ...
      const data = {
        ...
        hospitalId: hospitalId == -1 ? -1 : hospitalId,
      };
      api.hospitalDealPriceBoard(data).then((res) => {
        if (res.code === 0) {
  ...
  watch: {
    activeName: {
      handler(value) {
        if (value === "hospitalReconciliation") {
