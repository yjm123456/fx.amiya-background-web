<template>
  <div>
    <Card :dis-hover="true">
      ...
      ></DatePicker>
      <Button type="primary" style="margin-left: 10px" @click="getcustomerServiceBelongBoardData()"
        >查询</Button
      >
    </Card>
...
export default {
  ...
  methods: {
    ...
    // 获取客户看板列表
    getcustomerServiceBelongBoardData() {
      const {
        startDate,
        endDate,
        liveAnchorId,
      ...
      api.customerServiceBelongBoardData(data).then((res) => {
        if (res.code === 0) {
          const { data} = res.data;
  ...
  watch: {
    activeName: {
      handler(value) {
        if (value === "customerServicePerformance") {
          ...
          this.getcustomerServiceBelongBoardData();
        }
      },
      immediate: true,
    },
