<template>
  <div>
    <Card :dis-hover="true">
      ...
      >
        <Option
          v-for="item in subsidiaryParams.companyNameAllList"
          :value="item.id"
          :key="item.id"
          >{{ item.name }}</Option
        >
      ...
      >
        <Option
          v-for="item in subsidiaryParams.hospitalAllList"
          :value="item.id"
          :key="item.id"
          >{{ item.name }}</Option
        >
...
export default {
  ...
  methods: {
    ...
    getSubsidiary() {
      ...
      api.getSubsidiaryBoard(data).then((res) => {
        if (res.code === 0) {
