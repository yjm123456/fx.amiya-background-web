...
export default {
  ...
  methods: {
    // 业绩
    myEcharts(value) {
      const {clueData,performanceData,newCustomerPerformanceData,oldCustomerPerformanceData} = value
      let date =[]
      let clueDatas =[]
      let performanceDatas =[]
      let newCustomerPerformanceDatas =[]
      let oldCustomerPerformanceDatas =[]
      ...
      newCustomerPerformanceData ? newCustomerPerformanceData.map(item=>{
        newCustomerPerformanceDatas.push(item.performance)
      }):[]
      oldCustomerPerformanceData ? oldCustomerPerformanceData.map(item=>{
        oldCustomerPerformanceDatas.push(item.performance)
      }):[]
      let title = '线索'
      let title2 = '业绩'
      let option = {
        ...
        series: [
            ...
            },
            {
                name: '新客业绩',
                type: 'line',
                data: newCustomerPerformanceDatas,
                itemStyle: { 
                    normal: { 
                        color: '#BB5DF9'  // 折线的颜⾊
                    } 
            ...
            },
            {
                name: '老客业绩',
                type: 'line',
                data: oldCustomerPerformanceDatas,
                itemStyle: { 
                    normal: { 
                        color: '#F37F51'  // 折线的颜⾊
                    } 
