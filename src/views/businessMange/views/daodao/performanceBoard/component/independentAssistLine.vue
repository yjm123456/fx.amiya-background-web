...
export default {
  props: {
    daodaoindependentAssist:Object
  },
  data() {
    return {
      myChart: "",
  ...
  methods: {
    // 业绩
    myEcharts(value) {
      const {liveAnchorIndenpendentPerformanceBrokenLine,customerServiceIndenpendentPerformanceBrokenLine,customerServiceAssistPerformanceBrokenLine} = value
      let date =[]
      let liveAnchorIndenpendentPerformanceBrokenLines =[]
      let customerServiceIndenpendentPerformanceBrokenLines =[]
      let customerServiceAssistPerformanceBrokenLines =[]
      liveAnchorIndenpendentPerformanceBrokenLine.map(item=>{
        date.push(item.date)
        liveAnchorIndenpendentPerformanceBrokenLines.push(item.performance)
      })
      customerServiceIndenpendentPerformanceBrokenLine.map(item=>{
        customerServiceIndenpendentPerformanceBrokenLines.push(item.performance)
      })
      customerServiceAssistPerformanceBrokenLine.map(item=>{
        customerServiceAssistPerformanceBrokenLines.push(item.performance)
      })
      let option = {
        title: {
          text: "趋势图",
        ...
        legend: {
          x:'center',
          y:'bottom',
          data: ['主播独立业绩', '助理独立业绩','助理协助业绩'],
        },
        series: [
           {
                name: '主播独立业绩',
                type: 'line',
                data: liveAnchorIndenpendentPerformanceBrokenLines,
                itemStyle: { 
                    normal: { 
                        color: '#2d8cf0'  // 折线的颜⾊
                    } 
            ...
            },
            {
                name: '助理独立业绩',
                type: 'line',
                data: customerServiceIndenpendentPerformanceBrokenLines,
                itemStyle: { 
                    normal: { 
                        color: '#19be6b'  // 折线的颜⾊
                    } 
            ...
            },
            {
                name: '助理协助业绩',
                type: 'line',
                data: customerServiceAssistPerformanceBrokenLines,
                itemStyle: { 
                    normal: { 
                        color: '#ff9900'  // 折线的颜⾊
                    } 
