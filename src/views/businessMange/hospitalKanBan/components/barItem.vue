  ...
  methods: {
    ...
    myEcharts(value2) {
      ...
      value2.map(item=>{
        ...
        totalCustomerPerformanceList.unshift(item.totalPerformance)
      })
      let option = {
        tooltip: {
          trigger: "axis",
        ...
        yAxis: {
          ...
          axisLabel: {
            formatter: function(value) {
                return value.length > 5 ? value.slice(0, 5) + '...'  + '(' +  value2.find(item=>item.name == value).totalPerformance + 'w)' : value + '(' +  value2.find(item=>item.name == value).totalPerformance + 'w)'
                // 
            }
        }
        },
